...
BEGIN
  ...
  IF acc_ap IS NOT NULL THEN
    ...
    LOOP
      ...
      -- AP credit MUST equal the sum of debit/credit lines above to guarantee balance.
      -- Using the sum of rounded line totals (not invoice.total_amount) eliminates
      -- per-line rounding drift that caused "debits=741.80, credits=741.81" failures.
      ...
      -- Catch any residual cent drift (defensive — should be zero after the fix above)
      SELECT COALESCE(SUM(debit),0), COALESCE(SUM(credit),0)
        INTO v_total_debits, v_total_credits
        FROM public.journal_lines WHERE entry_id = e_id;
      v_imbalance := ROUND(v_total_debits - v_total_credits, 2);
