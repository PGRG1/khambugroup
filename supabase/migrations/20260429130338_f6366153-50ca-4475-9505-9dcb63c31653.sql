...
-- Seed payroll mapping defaults (use existing constraint-allowed rule types)
DO $$
DECLARE
  v_sal_exp uuid;
  v_mpf_exp uuid;
  ...
  v_cash    uuid;
BEGIN
  SELECT id INTO v_sal_exp FROM public.chart_of_accounts WHERE code='6010' LIMIT 1;
  SELECT id INTO v_mpf_exp FROM public.chart_of_accounts WHERE code='6020' LIMIT 1;
  SELECT id INTO v_sal_pay FROM public.chart_of_accounts WHERE code='2040' LIMIT 1;
  SELECT id INTO v_mpf_pay FROM public.chart_of_accounts WHERE code='2030' LIMIT 1;
  SELECT id INTO v_cash    FROM public.chart_of_accounts WHERE is_cash = true ORDER BY code LIMIT 1;

  IF v_sal_exp IS NOT NULL THEN
    INSERT INTO public.account_mapping_rules (rule_type, match_key, account_id, notes)
    VALUES ('payroll_salary_expense', '', v_sal_exp, 'Default Salaries Expense (global)')
    ON CONFLICT (rule_type, match_key) DO NOTHING;
  END IF;
  IF v_mpf_exp IS NOT NULL THEN
    INSERT INTO public.account_mapping_rules (rule_type, match_key, account_id, notes)
    VALUES ('payroll_mpf_expense', '', v_mpf_exp, 'Default MPF Expense (global)')
    ON CONFLICT (rule_type, match_key) DO NOTHING;
  END IF;
  IF v_sal_pay IS NOT NULL THEN
    INSERT INTO public.account_mapping_rules (rule_type, match_key, account_id, notes)
    VALUES ('salary_payable', '', v_sal_pay, 'Default Salary Payable')
    ON CONFLICT (rule_type, match_key) DO NOTHING;
  END IF;
  IF v_mpf_pay IS NOT NULL THEN
    INSERT INTO public.account_mapping_rules (rule_type, match_key, account_id, notes)
    VALUES ('mpf_payable', '', v_mpf_pay, 'Default MPF Payable')
    ON CONFLICT (rule_type, match_key) DO NOTHING;
  END IF;
  IF v_cash IS NOT NULL THEN
    INSERT INTO public.account_mapping_rules (rule_type, match_key, account_id, notes) VALUES
      ('payment_method_cash', 'bank_transfer', v_cash, 'Default cash for bank_transfer'),
      ('payment_method_cash', 'cash',          v_cash, 'Default cash for cash payment'),
      ('payment_method_cash', 'cheque',        v_cash, 'Default cash for cheque')
    ON CONFLICT (rule_type, match_key) DO NOTHING;
  END IF;
END $$;

-- Extend rebuild_journal_from_operations with payroll
  ...
  IF acc_pr_sal_pay IS NOT NULL AND acc_pr_mpf_pay IS NOT NULL THEN
    FOR pr IN
      SELECT p.id, p.year, p.month,
             ROUND(COALESCE(p.gross_salary,0)::numeric,2)        AS gross_salary,
             ROUND(COALESCE(p.net_salary,0)::numeric,2)          AS net_salary,
             ROUND(COALESCE(p.mpf_employee,0)::numeric,2)        AS mpf_employee,
             ROUND(COALESCE(p.mpf_employer,0)::numeric,2)        AS mpf_employer,
    ...
    LOOP
      ...
      v_gross := pr.gross_salary;
      v_net   := pr.net_salary;
      v_mpf_e := pr.mpf_employee;
      v_mpf_r := pr.mpf_employer;
      ...
      -- Entry A: monthly accrual (last day of payroll month)
      v_accrual_date := (make_date(pr.year, pr.month, 1) + INTERVAL '1 month - 1 day')::date;

      INSERT INTO public.journal_entries (entry_date, memo, source_type, source_id, venue, status)
        VALUES (v_accrual_date,
                'Payroll accrual '||to_char(make_date(pr.year, pr.month, 1),'YYYY-MM')||' — '||pr.emp_name,
      ...
      -- Need at least 2 lines or check_journal_balance trigger will reject
      ...
      -- Entry B: net salary cash payment
      IF pr.net_salary_payment_date IS NOT NULL AND v_net > 0 THEN
        acc_pay_cash := NULL;
        SELECT account_id INTO acc_pay_cash FROM public.account_mapping_rules
        ...
        IF acc_pay_cash IS NOT NULL THEN
          INSERT INTO public.journal_entries (entry_date, memo, source_type, source_id, venue, status)
            VALUES (pr.net_salary_payment_date,
                    'Payroll net pay '||to_char(make_date(pr.year, pr.month, 1),'YYYY-MM')||' — '||pr.emp_name,
      ...
      IF pr.mpf_payment_date IS NOT NULL AND v_mpf_total > 0 THEN
        ...
        IF acc_pay_cash IS NOT NULL THEN
          INSERT INTO public.journal_entries (entry_date, memo, source_type, source_id, venue, status)
            VALUES (pr.mpf_payment_date,
                    'MPF remittance '||to_char(make_date(pr.year, pr.month, 1),'YYYY-MM')||' — '||pr.emp_name,
