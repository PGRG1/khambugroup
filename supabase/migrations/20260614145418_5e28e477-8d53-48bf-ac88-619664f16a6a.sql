...
BEGIN
  ...
  LOOP
    IF EXISTS (SELECT 1 FROM public.journal_entries
               WHERE source_type='sales_summary'
                 ...
                 AND manually_adjusted = true) THEN
      CONTINUE;
    END IF;
    ...
    INSERT INTO public.journal_entries (entry_date,memo,source_type,source_id,venue,status)
      VALUES (r.entry_date,'Sales summary — '||r.venue,'sales_summary',r.entry_date::text||'__'||r.venue,r.venue,'draft')
    ...
    IF r.subtotal > 0 AND acc_sales IS NOT NULL THEN
      ...
      INSERT INTO public.journal_lines (entry_id,account_id,debit,credit,venue,line_no,memo)
        VALUES (e_id,acc_sales,0,r.subtotal,r.venue,v_ln,'Sales');
    ELSIF r.subtotal < 0 AND acc_sales IS NOT NULL THEN
      v_ln := v_ln+1;
      INSERT INTO public.journal_lines (entry_id,account_id,debit,credit,venue,line_no,memo)
        VALUES (e_id,acc_sales,ABS(r.subtotal),0,r.venue,v_ln,'Sales reversal');
    END IF;

    IF r.service_charge > 0 AND acc_svc IS NOT NULL THEN
      v_ln := v_ln+1;
      INSERT INTO public.journal_lines (entry_id,account_id,debit,credit,venue,line_no,memo)
        VALUES (e_id,acc_svc,0,r.service_charge,r.venue,v_ln,'Service charge');
    ELSIF r.service_charge < 0 AND acc_svc IS NOT NULL THEN
      v_ln := v_ln+1;
      INSERT INTO public.journal_lines (entry_id,account_id,debit,credit,venue,line_no,memo)
        VALUES (e_id,acc_svc,ABS(r.service_charge),0,r.venue,v_ln,'Service charge reversal');
    END IF;
    ...
    IF r.card_tips > 0 AND acc_tips IS NOT NULL THEN
      v_ln := v_ln+1;
      INSERT INTO public.journal_lines (entry_id,account_id,debit,credit,venue,line_no,memo)
        VALUES (e_id,acc_tips,0,r.card_tips,r.venue,v_ln,'Card tips');
    ELSIF r.card_tips < 0 AND acc_tips IS NOT NULL THEN
      v_ln := v_ln+1;
      INSERT INTO public.journal_lines (entry_id,account_id,debit,credit,venue,line_no,memo)
        VALUES (e_id,acc_tips,ABS(r.card_tips),0,r.venue,v_ln,'Card tips reversal');
    END IF;

    IF r.m_cash > 0 AND acc_cash IS NOT NULL THEN
      v_ln := v_ln+1;
      INSERT INTO public.journal_lines (entry_id,account_id,debit,credit,venue,line_no,memo)
        VALUES (e_id,acc_cash,r.m_cash,0,r.venue,v_ln,'Cash receipts');
    ELSIF r.m_cash < 0 AND acc_cash IS NOT NULL THEN
      v_ln := v_ln+1;
      INSERT INTO public.journal_lines (entry_id,account_id,debit,credit,venue,line_no,memo)
        VALUES (e_id,acc_cash,0,ABS(r.m_cash),r.venue,v_ln,'Cash receipts reversal');
    END IF;
    ...
    FOREACH v_method IN ARRAY v_methods LOOP
      ...
      IF v_amt IS NOT NULL AND v_amt <> 0 THEN
        ...
        SELECT account_id INTO acc_pm FROM public.account_mapping_rules
          WHERE rule_type=v_method AND match_key=r.venue LIMIT 1;
        ...
        IF acc_pm IS NOT NULL THEN
          ...
          IF v_amt > 0 THEN
            INSERT INTO public.journal_lines (entry_id,account_id,debit,credit,venue,line_no,memo)
              VALUES (e_id,acc_pm,v_amt,0,r.venue,v_ln,COALESCE(v_labels->>v_method, v_method)||' receivable');
          ELSE
            INSERT INTO public.journal_lines (entry_id,account_id,debit,credit,venue,line_no,memo)
              VALUES (e_id,acc_pm,0,ABS(v_amt),r.venue,v_ln,COALESCE(v_labels->>v_method, v_method)||' receivable reversal');
          END IF;
        END IF;
      END IF;
    END LOOP;
    ...
    IF v_imbalance <> 0 AND acc_suspense IS NOT NULL THEN
      ...
      IF v_imbalance > 0 THEN
        INSERT INTO public.journal_lines (entry_id,account_id,debit,credit,venue,line_no,memo)
          VALUES (e_id,acc_suspense,0,v_imbalance,r.venue,v_ln,'Δ');
      ELSE
        INSERT INTO public.journal_lines (entry_id,account_id,debit,credit,venue,line_no,memo)
          VALUES (e_id,acc_suspense,-v_imbalance,0,r.venue,v_ln,'Δ');
      END IF;
    END IF;
  ...
  LOOP
    ...
    LOOP
      ...
      IF v_pm_key = '' THEN CONTINUE; END IF;
      acc_pm := NULL;
      SELECT account_id INTO acc_pm FROM public.account_mapping_rules
       WHERE rule_type=v_pm_key AND match_key=sb.merchant_venue LIMIT 1;
      IF acc_pm IS NULL THEN
        SELECT account_id INTO acc_pm FROM public.account_mapping_rules
         WHERE rule_type='sales_payment_method' AND match_key=v_pm_key||'__'||sb.merchant_venue LIMIT 1;
      END IF;
      IF acc_pm IS NULL THEN
        SELECT account_id INTO acc_pm FROM public.account_mapping_rules
         WHERE rule_type='sales_payment_method' AND match_key=v_pm_key LIMIT 1;
      END IF;
      IF acc_pm IS NULL THEN
        SELECT account_id INTO acc_pm FROM public.account_mapping_rules
      ...
      IF acc_pm IS NOT NULL AND sl.amount <> 0 THEN
        ...
        IF ROUND(sl.amount::numeric,2) > 0 THEN
          INSERT INTO public.journal_lines (entry_id,account_id,debit,credit,venue,line_no,memo)
          VALUES (e_id, acc_pm, 0, ROUND(sl.amount::numeric,2), sb.merchant_venue, v_ln, COALESCE(v_labels->>v_pm_key, v_pm_key)||' clearing');
        ELSE
          INSERT INTO public.journal_lines (entry_id,account_id,debit,credit,venue,line_no,memo)
          VALUES (e_id, acc_pm, ABS(ROUND(sl.amount::numeric,2)), 0, sb.merchant_venue, v_ln, COALESCE(v_labels->>v_pm_key, v_pm_key)||' clearing reversal');
        END IF;
      END IF;
    END LOOP;
