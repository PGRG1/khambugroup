...
ALTER TABLE public.account_mapping_rules
  ADD CONSTRAINT account_mapping_rules_rule_type_check
  CHECK (rule_type = ANY (ARRAY[
    'sales_revenue','service_charge','sales_cash','sales_payment_method','sales_discount','tips_payable',
    ...
    'manual_income','manual_expense','opening_equity','processor_fees','procurement_category',
    'bank_txn_type'
  ]));

-- 1) New COA: non-operating expense for bank-transfer fees
INSERT INTO public.chart_of_accounts (code, name, account_type, normal_side, is_active, is_cash, sort_order, description)
SELECT '7110', 'Bank Charges – Bank Transfers', 'other_expense', 'debit', true, false, 7110, 'FPS / wire / bank-transfer charges (non-operating)'
WHERE NOT EXISTS (SELECT 1 FROM public.chart_of_accounts WHERE code='7110');

-- 2) Allow bank_fee as journal source
...
ALTER TABLE public.journal_entries
  ...
  CHECK (source_type = ANY (ARRAY[
    ...
    'settlement_fee','bank_fee',
    'manual','adjustment','opening'
  ]));

-- 3) Default mapping for bank_fee → COA 7110
INSERT INTO public.account_mapping_rules (rule_type, match_key, account_id, notes)
SELECT 'bank_txn_type', 'bank_fee', (SELECT id FROM public.chart_of_accounts WHERE code='7110'), 'Default account for FPS/bank transfer fees'
WHERE NOT EXISTS (
  SELECT 1 FROM public.account_mapping_rules WHERE rule_type='bank_txn_type' AND match_key='bank_fee'
);

-- 4) Update rebuild function: same body + new BANK FEE block at the end
...
DECLARE
  ...
  v_lines_sum numeric;
  v_ap_amount numeric;
  pr record; sb record; bf record;
  v_accrual_date date;
  v_gross numeric; v_net numeric; v_mpf_e numeric; v_mpf_r numeric; v_mpf_total numeric;
  v_proc_fee numeric; v_xfer_fee numeric; v_fees_total numeric;
  v_emp_venue text;
  v_uid uuid := auth.uid();
  v_uname text;
  v_period text;
...
BEGIN
  ...
  IF acc_ap IS NOT NULL THEN
    ...
    LOOP
      ...
      LOOP
        IF line.total <> 0 AND line.acct IS NOT NULL THEN
          v_ln := v_ln+1;
  ...
  IF acc_pr_sal_pay IS NOT NULL AND acc_pr_mpf_pay IS NOT NULL THEN
    ...
    LOOP
      v_emp_venue := COALESCE(pr.emp_venue,''); v_period := to_char(make_date(pr.year,pr.month,1),'YYYY-MM');
      v_gross := COALESCE(NULLIF(pr.gross_salary,0),NULLIF(pr.actual_total,0),NULLIF(pr.forecast_total,0),NULLIF(pr.actual_base_salary,0),NULLIF(pr.forecast_base_salary,0),0);
      v_gross := ROUND(COALESCE(v_gross,0)::numeric,2);
      IF v_gross <= 0 THEN CONTINUE; END IF;
      v_mpf_e := ROUND(COALESCE(NULLIF(pr.mpf_employee,0), LEAST(MPF_CAP,v_gross*MPF_RATE))::numeric,2);
      v_mpf_r := ROUND(COALESCE(NULLIF(pr.mpf_employer,0), LEAST(MPF_CAP,v_gross*MPF_RATE))::numeric,2);
      v_mpf_total := ROUND(v_mpf_e+v_mpf_r,2);
      v_net := ROUND(COALESCE(NULLIF(pr.net_salary,0), v_gross - v_mpf_e)::numeric,2);
      acc_pr_sal_exp := NULL;
      SELECT account_id INTO acc_pr_sal_exp FROM public.account_mapping_rules WHERE rule_type='payroll_salary_expense' AND match_key=v_emp_venue LIMIT 1;
      IF acc_pr_sal_exp IS NULL THEN SELECT account_id INTO acc_pr_sal_exp FROM public.account_mapping_rules WHERE rule_type='payroll_salary_expense' AND match_key='' LIMIT 1; END IF;
      acc_pr_mpf_exp := NULL;
      SELECT account_id INTO acc_pr_mpf_exp FROM public.account_mapping_rules WHERE rule_type='payroll_mpf_expense' AND match_key=v_emp_venue LIMIT 1;
      IF acc_pr_mpf_exp IS NULL THEN SELECT account_id INTO acc_pr_mpf_exp FROM public.account_mapping_rules WHERE rule_type='payroll_mpf_expense' AND match_key='' LIMIT 1; END IF;
      ...
      IF NOT EXISTS (SELECT 1 FROM public.journal_entries WHERE source_type='payroll_accrual' AND source_id=pr.id::text AND manually_adjusted=true) THEN
        INSERT INTO public.journal_entries (entry_date,memo,source_type,source_id,venue,status)
          VALUES (v_accrual_date,'Payroll accrual '||v_period||' — '||pr.emp_name,'payroll_accrual',pr.id::text,NULLIF(v_emp_venue,''),'draft') RETURNING id INTO e_id;
        v_ln:=0;
      ...
      IF v_net>0 AND (pr.net_salary_payment_date IS NOT NULL OR pr.payment_status='paid') THEN
        IF NOT EXISTS (SELECT 1 FROM public.journal_entries WHERE source_type='payroll_payment' AND source_id=pr.id::text AND manually_adjusted=true) THEN
          ...
          IF acc_pay_cash IS NOT NULL THEN
            INSERT INTO public.journal_entries (entry_date,memo,source_type,source_id,venue,status) VALUES (COALESCE(pr.net_salary_payment_date,v_accrual_date),'Payroll net pay '||v_period||' — '||pr.emp_name,'payroll_payment',pr.id::text,NULLIF(v_emp_venue,''),'draft') RETURNING id INTO e_id;
      ...
      IF v_mpf_total>0 AND (pr.mpf_payment_date IS NOT NULL OR pr.payment_status='paid') THEN
        IF NOT EXISTS (SELECT 1 FROM public.journal_entries WHERE source_type='mpf_payment' AND source_id=pr.id::text AND manually_adjusted=true) THEN
          ...
          IF acc_pay_cash IS NULL THEN SELECT id INTO acc_pay_cash FROM public.chart_of_accounts WHERE is_cash=true ORDER BY code LIMIT 1; END IF;
          IF acc_pay_cash IS NOT NULL THEN
            INSERT INTO public.journal_entries (entry_date,memo,source_type,source_id,venue,status) VALUES (COALESCE(pr.mpf_payment_date,v_accrual_date),'MPF remittance '||v_period||' — '||pr.emp_name,'mpf_payment',pr.id::text,NULLIF(v_emp_venue,''),'draft') RETURNING id INTO e_id;
  ...
  -- ===== PAYMENT PROCESSOR SETTLEMENT FEES =====
  FOR sb IN
    SELECT b.id,b.settlement_date,b.fee_amount,b.bank_transfer_fee, m.display_name AS merchant_name, m.venue AS merchant_venue, m.fee_account_id, COALESCE(b.bank_account_id,m.default_bank_account_id) AS bank_acc_id, p.name AS proc_name
    ...
    WHERE (b.fee_amount<>0 OR b.bank_transfer_fee<>0)
  LOOP
    IF EXISTS (SELECT 1 FROM public.journal_entries WHERE source_type='settlement_fee' AND source_id=sb.id::text AND manually_adjusted=true) THEN CONTINUE; END IF;
    v_proc_fee := ROUND(ABS(COALESCE(sb.fee_amount,0))::numeric,2);
    v_xfer_fee := ROUND(ABS(COALESCE(sb.bank_transfer_fee,0))::numeric,2);
    v_fees_total := ROUND(v_proc_fee+v_xfer_fee,2);
    IF v_fees_total<=0 THEN CONTINUE; END IF;
    acc_bank := NULL;
    IF sb.bank_acc_id IS NOT NULL THEN SELECT linked_gl_account_id INTO acc_bank FROM public.bank_accounts WHERE id=sb.bank_acc_id; END IF;
    IF acc_bank IS NULL THEN CONTINUE; END IF;
    acc_fee := COALESCE(sb.fee_account_id, acc_proc_fee_default);
    IF acc_fee IS NULL THEN CONTINUE; END IF;
    INSERT INTO public.journal_entries (entry_date,memo,source_type,source_id,venue,status)
      VALUES (sb.settlement_date, sb.proc_name||' fees — '||sb.merchant_name,'settlement_fee',sb.id::text,sb.merchant_venue,'draft') RETURNING id INTO e_id;
    v_ln:=0;
    IF v_proc_fee>0 THEN v_ln:=v_ln+1; INSERT INTO public.journal_lines(entry_id,account_id,debit,credit,venue,line_no,memo) VALUES(e_id,acc_fee,v_proc_fee,0,sb.merchant_venue,v_ln,'Processing fee'); END IF;
    IF v_xfer_fee>0 THEN v_ln:=v_ln+1; INSERT INTO public.journal_lines(entry_id,account_id,debit,credit,venue,line_no,memo) VALUES(e_id,COALESCE(acc_bank_fee_default,acc_fee),v_xfer_fee,0,sb.merchant_venue,v_ln,'Bank transfer fee'); END IF;
    v_ln:=v_ln+1; INSERT INTO public.journal_lines(entry_id,account_id,debit,credit,venue,line_no,memo) VALUES(e_id,acc_bank,0,v_fees_total,sb.merchant_venue,v_ln,'Fees deducted at settlement');
    UPDATE public.journal_entries SET status='posted' WHERE id=e_id;
    cnt:=cnt+1;
  END LOOP;

  -- ===== BANK TRANSACTIONS: bank fees (FPS / wire / charges) =====
  FOR bf IN
