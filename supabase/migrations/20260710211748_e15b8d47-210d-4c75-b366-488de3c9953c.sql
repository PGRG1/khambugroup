...
BEGIN
  ...
  INSERT INTO public.journal_entries (tenant_id, entry_date, description, source, source_id, created_by, status)
  VALUES (v_tenant, v_bill.bill_date, 'Bill: ' || COALESCE(v_bill.vendor_name,'') || ' ' || COALESCE(v_bill.bill_number,''),
          'expense_bill', v_bill.id, v_actor, 'posted')
  RETURNING id INTO v_je_id;

  INSERT INTO public.journal_lines (tenant_id, journal_entry_id, account_id, debit, credit, memo, venue, department, line_no)
  SELECT v_tenant, v_je_id, a.account_id, a.amount, 0, COALESCE(a.notes, a.expense_category), a.venue, a.department, a.line_no
  FROM public.expense_bill_allocations a
  WHERE a.bill_id = p_bill_id
  ORDER BY a.line_no;

  INSERT INTO public.journal_lines (tenant_id, journal_entry_id, account_id, debit, credit, memo, line_no)
...
BEGIN
  ...
  IF v_stmt.approval_status = 'posted' THEN RETURN v_stmt.journal_entry_id; END IF;
  ...
  INSERT INTO public.journal_entries (tenant_id, entry_date, description, source, source_id, created_by, status)
  VALUES (v_stmt.tenant_id, v_stmt.statement_date, 'Vendor statement: ' || COALESCE(v_stmt.vendor_name,''),
          'vendor_statement', v_stmt.id, v_actor, 'posted')
  RETURNING id INTO v_je_id;
  ...
  UPDATE public.expense_vendor_statements
     SET approval_status = 'posted', journal_entry_id = v_je_id
