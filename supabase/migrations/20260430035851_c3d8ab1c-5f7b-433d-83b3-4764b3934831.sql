...
BEGIN
  ...
  IF acc_pr_sal_pay IS NOT NULL AND acc_pr_mpf_pay IS NOT NULL THEN
    ...
    LOOP
      ...
      -- Effective gross: prefer explicit gross_salary, else actual_total, else forecast_total
      ...
      -- MPF: prefer stored, else compute 5% capped at 1500 each side
      ...
      -- Net: prefer stored, else gross - employee MPF
      ...
      -- Entry B: net salary cash payment (date OR payment_status='paid' fallback)
      ...
      -- Entry C: MPF cash payment (date OR payment_status='paid' fallback)
