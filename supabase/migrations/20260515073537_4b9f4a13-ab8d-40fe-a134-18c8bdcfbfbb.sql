...
-- 1. discount_type on invoices
ALTER TABLE public.invoices
  ADD COLUMN IF NOT EXISTS discount_type text NOT NULL DEFAULT 'discount';

ALTER TABLE public.invoices
  DROP CONSTRAINT IF EXISTS invoices_discount_type_chk;
ALTER TABLE public.invoices
  ADD CONSTRAINT invoices_discount_type_chk
  CHECK (discount_type IN ('discount','refund'));

-- 2. Extend rule_type allow-list
ALTER TABLE public.account_mapping_rules
  DROP CONSTRAINT IF EXISTS account_mapping_rules_rule_type_check;
ALTER TABLE public.account_mapping_rules
  ...
  CHECK (rule_type = ANY (ARRAY[
    ...
    'tips_payable','payment_method_cash','invoice_expense','accounts_payable',
    ...
    'procurement_category','bank_txn_type','invoice_discount','invoice_refund'
  ]));

-- 3. New COA accounts
INSERT INTO public.chart_of_accounts (code, name, account_type, normal_side, is_active, sort_order)
VALUES
  ('5900', 'Purchase Discounts Received', 'cogs', 'credit', true, 5900),
  ('5910', 'Supplier Refunds',            'cogs', 'credit', true, 5910)
ON CONFLICT (code) DO NOTHING;

-- 4. Default mapping rules
INSERT INTO public.account_mapping_rules (rule_type, match_key, account_id, notes)
SELECT 'invoice_discount', '', id, 'Default account for invoice-level discounts'
FROM public.chart_of_accounts WHERE code = '5900'
ON CONFLICT DO NOTHING;

INSERT INTO public.account_mapping_rules (rule_type, match_key, account_id, notes)
SELECT 'invoice_refund', '', id, 'Default account for invoice-level supplier refunds'
FROM public.chart_of_accounts WHERE code = '5910'
ON CONFLICT DO NOTHING;

-- 5. Update rebuild_journal_from_operations to post invoice-level discounts/refunds
...
BEGIN
  ...
  -- ===== SALES SUMMARIES =====
  FOR r IN
    SELECT s.date::date AS date, s.venue,
           ...
           ROUND(COALESCE(SUM(s.cash),0)::numeric,2) AS cash,
           ROUND(COALESCE(SUM(s.visa),0)::numeric,2) AS visa,
           ROUND(COALESCE(SUM(s.mastercard),0)::numeric,2) AS mastercard,
           ROUND(COALESCE(SUM(s.amex),0)::numeric,2) AS amex,
           ROUND(COALESCE(SUM(s.union_pay),0)::numeric,2) AS union_pay,
           ROUND(COALESCE(SUM(s.jcb),0)::numeric,2) AS jcb,
           ROUND(COALESCE(SUM(s.alipay),0)::numeric,2) AS alipay,
           ROUND(COALESCE(SUM(s.wechat),0)::numeric,2) AS wechat,
           ROUND(COALESCE(SUM(s.payme),0)::numeric,2) AS payme
    FROM public.sales_data s
    GROUP BY s.date, s.venue
  ...
  LOOP
    ...
    SELECT account_id INTO acc_tips  FROM public.account_mapping_rules WHERE rule_type='card_tips'      AND match_key=r.venue LIMIT 1;
    SELECT account_id INTO acc_cash  FROM public.account_mapping_rules WHERE rule_type='cash_clearing'  AND match_key=r.venue LIMIT 1;

    INSERT INTO public.journal_entries (entry_date,memo,source_type,source_id,venue,status)
      VALUES (r.date,'Sales summary — '||r.venue,'sales_summary',r.date::text||'__'||r.venue,r.venue,'draft') RETURNING id INTO e_id;
    v_ln := 0;
    IF r.subtotal <> 0 AND acc_sales IS NOT NULL THEN
      v_ln := v_ln+1; INSERT INTO public.journal_lines (entry_id,account_id,debit,credit,venue,line_no,memo) VALUES (e_id,acc_sales,0,r.subtotal,r.venue,v_ln,'Sales');
    END IF;
    IF r.service_charge <> 0 AND acc_svc IS NOT NULL THEN
      v_ln := v_ln+1; INSERT INTO public.journal_lines (entry_id,account_id,debit,credit,venue,line_no,memo) VALUES (e_id,acc_svc,0,r.service_charge,r.venue,v_ln,'Service charge');
    END IF;
    ...
    IF r.card_tips <> 0 AND acc_tips IS NOT NULL THEN
      v_ln := v_ln+1; INSERT INTO public.journal_lines (entry_id,account_id,debit,credit,venue,line_no,memo) VALUES (e_id,acc_tips,0,r.card_tips,r.venue,v_ln,'Card tips');
    END IF;
    IF r.cash <> 0 AND acc_cash IS NOT NULL THEN
      v_ln := v_ln+1; INSERT INTO public.journal_lines (entry_id,account_id,debit,credit,venue,line_no,memo) VALUES (e_id,acc_cash,r.cash,0,r.venue,v_ln,'Cash receipts');
    END IF;
    FOREACH v_method IN ARRAY v_methods LOOP
      ...
      IF v_amt IS NOT NULL AND v_amt <> 0 THEN
        SELECT account_id INTO acc_pm FROM public.account_mapping_rules WHERE rule_type=v_method AND match_key=r.venue LIMIT 1;
        IF acc_pm IS NOT NULL THEN
          v_ln := v_ln+1; INSERT INTO public.journal_lines (entry_id,account_id,debit,credit,venue,line_no,memo) VALUES (e_id,acc_pm,v_amt,0,r.venue,v_ln,(v_labels->>v_method)||' receivable');
        END IF;
      END IF;
    END LOOP;
    ...
    IF v_imbalance <> 0 AND acc_suspense IS NOT NULL THEN
      v_ln := v_ln+1;
      IF v_imbalance > 0 THEN INSERT INTO public.journal_lines (entry_id,account_id,debit,credit,venue,line_no,memo) VALUES (e_id,acc_suspense,0,v_imbalance,r.venue,v_ln,'Δ');
      ELSE INSERT INTO public.journal_lines (entry_id,account_id,debit,credit,venue,line_no,memo) VALUES (e_id,acc_suspense,-v_imbalance,0,r.venue,v_ln,'Δ'); END IF;
    END IF;
  ...
  IF acc_ap IS NOT NULL THEN
    ...
    LOOP
      ...
      -- Invoice-level discount or refund (contra-COGS credit)
    ...
    LOOP
      ...
      v_ln := 0;
      v_ln := v_ln+1; INSERT INTO public.journal_lines (entry_id,account_id,debit,credit,venue,line_no,memo) VALUES (e_id,acc_ap,r.amount,0,r.venue,v_ln,'AP settle');
      IF r.payment_method='cash' AND acc_pay_cash IS NOT NULL THEN
        v_ln := v_ln+1; INSERT INTO public.journal_lines (entry_id,account_id,debit,credit,venue,line_no,memo) VALUES (e_id,acc_pay_cash,0,r.amount,r.venue,v_ln,'Cash payment');
      ELSE
        SELECT account_id INTO acc_bank FROM public.account_mapping_rules WHERE rule_type='bank_payment_clearing' AND (match_key=r.payment_method OR match_key='') ORDER BY (match_key=r.payment_method) DESC LIMIT 1;
        IF acc_bank IS NOT NULL THEN
          v_ln := v_ln+1; INSERT INTO public.journal_lines (entry_id,account_id,debit,credit,venue,line_no,memo) VALUES (e_id,acc_bank,0,r.amount,r.venue,v_ln,'Bank payment');
        ELSIF acc_suspense IS NOT NULL THEN
          v_ln := v_ln+1; INSERT INTO public.journal_lines (entry_id,account_id,debit,credit,venue,line_no,memo) VALUES (e_id,acc_suspense,0,r.amount,r.venue,v_ln,'Unmapped bank payment');
        END IF;
      END IF;
      UPDATE public.journal_entries SET status='posted' WHERE id=e_id;
      cnt := cnt+1;
  ...
  LOOP
    ...
    FOR sl IN
      SELECT l.amount, l.payment_method
      FROM public.payment_settlement_batch_lines l
    ...
    LOOP
      ...
      SELECT account_id INTO acc_pm FROM public.account_mapping_rules
       WHERE rule_type=v_pm_key AND match_key=sb.merchant_venue LIMIT 1;
      IF acc_pm IS NULL THEN
        SELECT account_id INTO acc_pm FROM public.account_mapping_rules
         WHERE rule_type=v_pm_key LIMIT 1;
      END IF;
      IF acc_pm IS NOT NULL AND sl.amount <> 0 THEN
        v_ln := v_ln+1;
        INSERT INTO public.journal_lines (entry_id,account_id,debit,credit,venue,line_no,memo)
        VALUES (e_id, acc_pm, 0, ROUND(sl.amount::numeric,2), sb.merchant_venue, v_ln, (v_labels->>v_pm_key)||' clearing');
      END IF;
    END LOOP;
  ...
  RETURN jsonb_build_object('success', true, 'entries_posted', cnt);
END;
$function$;
