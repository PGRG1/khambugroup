-- 1. Seed fee CoA accounts
INSERT INTO public.chart_of_accounts (code, name, account_type, normal_side, is_active, is_cash, description, sort_order)
VALUES
  ('6810', 'Merchant Processing Fees', 'opex', 'debit', true, false, 'Card / wallet processor discount and processing fees', 681),
  ('6820', 'Settlement / Bank Transfer Fees', 'opex', 'debit', true, false, 'Per-settlement bank transfer fees from payment processors', 682)
ON CONFLICT (code) DO NOTHING;

-- 2. Recreate rebuild_journal_from_operations adding settlement-fees block
...
BEGIN
  ...
  INSERT INTO public.ledger_audit_log (event_type, user_id, user_display_name, status, notes)
    VALUES ('ledger_rebuild_start', v_uid, v_uname, 'in_progress', 'Rebuilding journal from operations (preserving manually-adjusted entries)');
  ...
  LOOP
    IF EXISTS (SELECT 1 FROM public.journal_entries
               WHERE source_type='sales' AND source_id = r.d::text||'|'||r.venue AND manually_adjusted = true) THEN
      CONTINUE;
    END IF;
  ...
  IF acc_ap IS NOT NULL THEN
    ...
    LOOP
      IF EXISTS (SELECT 1 FROM public.journal_entries
                 WHERE source_type='invoice' AND source_id = inv.id::text AND manually_adjusted = true) THEN
        CONTINUE;
      END IF;
    ...
    LOOP
      IF EXISTS (SELECT 1 FROM public.journal_entries
                 WHERE source_type='invoice_payment' AND source_id = r.id::text AND manually_adjusted = true) THEN
        CONTINUE;
      END IF;
  ...
  IF acc_pr_sal_pay IS NOT NULL AND acc_pr_mpf_pay IS NOT NULL THEN
    ...
    LOOP
      ...
      IF NOT EXISTS (SELECT 1 FROM public.journal_entries
                     WHERE source_type='payroll_accrual' AND source_id = pr.id::text AND manually_adjusted = true) THEN
        INSERT INTO public.journal_entries (entry_date, memo, source_type, source_id, venue, status)
      ...
      IF v_net > 0 AND (pr.net_salary_payment_date IS NOT NULL OR pr.payment_status = 'paid') THEN
        IF EXISTS (SELECT 1 FROM public.journal_entries
                   WHERE source_type='payroll_payment' AND source_id = pr.id::text AND manually_adjusted = true) THEN
          NULL;
        ELSE
          acc_pay_cash := NULL;
          SELECT account_id INTO acc_pay_cash FROM public.account_mapping_rules
          ...
          IF acc_pay_cash IS NOT NULL THEN
            INSERT INTO public.journal_entries (entry_date, memo, source_type, source_id, venue, status)
              VALUES (COALESCE(pr.net_salary_payment_date, v_accrual_date),
                      ...
                      'payroll_payment', pr.id::text, NULLIF(v_emp_venue,''), 'draft')
              RETURNING id INTO e_id;
            INSERT INTO public.journal_lines (entry_id, account_id, debit, credit, venue, line_no, memo)
      ...
      IF v_mpf_total > 0 AND (pr.mpf_payment_date IS NOT NULL OR pr.payment_status = 'paid') THEN
        IF EXISTS (SELECT 1 FROM public.journal_entries
                   WHERE source_type='mpf_payment' AND source_id = pr.id::text AND manually_adjusted = true) THEN
          NULL;
        ELSE
          acc_pay_cash := NULL;
          SELECT account_id INTO acc_pay_cash FROM public.account_mapping_rules
          ...
          IF acc_pay_cash IS NOT NULL THEN
            INSERT INTO public.journal_entries (entry_date, memo, source_type, source_id, venue, status)
              VALUES (COALESCE(pr.mpf_payment_date, v_accrual_date),
                      ...
                      'mpf_payment', pr.id::text, NULLIF(v_emp_venue,''), 'draft')
              RETURNING id INTO e_id;
            INSERT INTO public.journal_lines (entry_id, account_id, debit, credit, venue, line_no, memo)
  ...
  -- Sales JE books gross to bank/card account; this entry reduces bank by fees and books OpEx.
  FOR sb IN
    SELECT b.id, b.settlement_date, b.fee_amount, b.bank_transfer_fee,
  ...
  LOOP
    IF EXISTS (SELECT 1 FROM public.journal_entries
               WHERE source_type='settlement_fee' AND source_id = sb.id::text AND manually_adjusted = true) THEN
      CONTINUE;
    END IF;
    ...
    IF acc_bank IS NULL THEN
      INSERT INTO public.ledger_audit_log (event_type, user_id, user_display_name, status, notes)
        VALUES ('settlement_fee_skipped', v_uid, v_uname, 'skipped',
                'No bank GL for merchant '||sb.merchant_name||' settlement '||sb.settlement_date);
      CONTINUE;
    END IF;

    -- Resolve fee expense GL
    acc_fee := COALESCE(sb.fee_account_id, acc_proc_fee_default);
    IF acc_fee IS NULL THEN CONTINUE; END IF;

    INSERT INTO public.journal_entries (entry_date, memo, source_type, source_id, venue, status)
      VALUES (sb.settlement_date,
              sb.proc_name||' fees — '||sb.merchant_name,
              'settlement_fee', sb.id::text, sb.merchant_venue, 'draft')
      RETURNING id INTO e_id;
    ...
    IF v_proc_fee > 0 THEN
      v_ln := v_ln + 1;
      INSERT INTO public.journal_lines (entry_id, account_id, debit, credit, venue, line_no, memo)
        VALUES (e_id, acc_fee, v_proc_fee, 0, sb.merchant_venue, v_ln, 'Processing fee');
    END IF;
    IF v_xfer_fee > 0 THEN
      v_ln := v_ln + 1;
      INSERT INTO public.journal_lines (entry_id, account_id, debit, credit, venue, line_no, memo)
        VALUES (e_id, COALESCE(acc_bank_fee_default, acc_fee), v_xfer_fee, 0, sb.merchant_venue, v_ln, 'Bank transfer fee');
    END IF;

    v_ln := v_ln + 1;
    INSERT INTO public.journal_lines (entry_id, account_id, debit, credit, venue, line_no, memo)
      VALUES (e_id, acc_bank, 0, v_fees_total, sb.merchant_venue, v_ln, 'Fees deducted at settlement');
