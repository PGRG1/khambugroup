...
-- Seed procurement_category mapping rules from existing per-product defaults
INSERT INTO public.account_mapping_rules (rule_type, match_key, account_id, notes)
SELECT
  'procurement_category',
  pm.financial_treatment || '__' || pm.level1_category,
  (
    SELECT pm2.default_coa_account_id
    FROM public.product_master pm2
    WHERE pm2.financial_treatment = pm.financial_treatment
      AND pm2.level1_category = pm.level1_category
      AND pm2.default_coa_account_id IS NOT NULL
    GROUP BY pm2.default_coa_account_id
    ORDER BY COUNT(*) DESC
    LIMIT 1
  ),
  'Auto-seeded from product defaults'
FROM public.product_master pm
WHERE pm.financial_treatment <> ''
  AND COALESCE(pm.level1_category,'') <> ''
GROUP BY pm.financial_treatment, pm.level1_category
HAVING (
  SELECT pm2.default_coa_account_id
  FROM public.product_master pm2
  WHERE pm2.financial_treatment = pm.financial_treatment
...
-- Update rebuild function: invoice debit = product override → category mapping
...
BEGIN
  ...
  -- INVOICES: per-product override → procurement_category mapping
