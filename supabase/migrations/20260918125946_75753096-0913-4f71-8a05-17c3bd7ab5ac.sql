ALTER TABLE public.invoice_line_items
  ADD COLUMN IF NOT EXISTS scanned_description text,
  ADD COLUMN IF NOT EXISTS scanned_item_code text,
  ADD COLUMN IF NOT EXISTS printed_amount numeric,
  ADD COLUMN IF NOT EXISTS source_line_no text;

CREATE OR REPLACE FUNCTION public.create_scanner_invoice_with_attachments(p_tenant_id uuid, p_invoice jsonb, p_line_items jsonb, p_attachments jsonb)
 RETURNS uuid
 LANGUAGE plpgsql
 SECURITY DEFINER
...
BEGIN
  ...
  INSERT INTO public.invoices (
    id, supplier_id, supplier_account_id, venue, venue_id, legacy_venue_name,
    invoice_number, invoice_date, due_date, status, subtotal, tax_amount, total_amount,
    discount, discount_type, notes, entered_by, file_url, file_name, received_date,
    payment_status, payment_method, dispute_notes, review_status, exception_note,
    ai_suggestions, ai_anomaly, ai_extract_meta, tenant_id, discount_mode, discount_rate,
    has_disputes, disputed_amount, source_origin
  ) VALUES (
    v_id,
  ...
  IF p_line_items IS NOT NULL AND jsonb_typeof(p_line_items) = 'array'
    ...
    INSERT INTO public.invoice_line_items (
      invoice_id, tenant_id, description, item_code, pack_size, category_id,
      quantity, unit, weight, unit_price, tax_amount, total, discount,
      discount_mode, discount_rate, line_discount_amount, header_discount_share,
      net_unit_cost, notes, product_master_id, standard_product_id,
      normalized_unit_cost, pack_size_norm, unit_norm, ai_suggestion,
      accepted_qty, qty_difference, receiving_reason, receiving_note,
      accepted_price, price_disputed, is_free_unit_line, deal_id,
      scanned_description, scanned_item_code, printed_amount, source_line_no
    )
    SELECT
      v_id,
      p_tenant_id,
      COALESCE(NULLIF(btrim(x->>'description'), ''), 'Unnamed item'),
      COALESCE(x->>'item_code', ''),
      COALESCE(x->>'pack_size', ''),
      ...
      COALESCE(NULLIF(x->>'discount_mode', ''), 'fixed'),
      COALESCE((x->>'discount_rate')::numeric, 0),
      COALESCE((x->>'line_discount_amount')::numeric, 0),
      COALESCE((x->>'header_discount_share')::numeric, 0),
      COALESCE((x->>'net_unit_cost')::numeric, 0),
      x->>'notes',
      NULLIF(x->>'product_master_id', '')::uuid,
      NULLIF(x->>'standard_product_id', '')::uuid,
      NULLIF(x->>'normalized_unit_cost', '')::numeric,
      x->>'pack_size_norm',
      x->>'unit_norm',
      CASE WHEN jsonb_typeof(x->'ai_suggestion') IN ('object','array') THEN x->'ai_suggestion' ELSE NULL END,
      NULLIF(x->>'accepted_qty', '')::numeric,
      NULLIF(x->>'qty_difference', '')::numeric,
      NULLIF(x->>'receiving_reason', ''),
      NULLIF(x->>'receiving_note', ''),
      NULLIF(x->>'accepted_price', '')::numeric,
      COALESCE((x->>'price_disputed')::boolean, false),
      COALESCE((x->>'is_free_unit_line')::boolean, false),
      NULLIF(x->>'deal_id', '')::uuid,
      NULLIF(x->>'scanned_description', ''),
      NULLIF(x->>'scanned_item_code', ''),
      NULLIF(x->>'printed_amount', '')::numeric,
      NULLIF(x->>'source_line_no', '')
