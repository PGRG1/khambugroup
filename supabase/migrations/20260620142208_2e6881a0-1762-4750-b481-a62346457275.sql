...
-- 1. Mapping rule types ----------------------------------------------------
ALTER TABLE public.account_mapping_rules
  DROP CONSTRAINT IF EXISTS account_mapping_rules_rule_type_check;

ALTER TABLE public.account_mapping_rules
  ADD CONSTRAINT account_mapping_rules_rule_type_check CHECK (
    rule_type = ANY (ARRAY[
      'sales_revenue','service_charge','sales_cash','sales_payment_method',
      'sales_discount','tips_payable','payment_method_cash',
      'cash_on_hand','payment_settlement_clearing',
      ...
      'processor_fees','procurement_category','bank_txn_type',
      'invoice_discount','invoice_refund',
      'suspense','cash_payment_clearing','processor_fee_default',
      'bank_transfer_fee_default','cash_clearing','bank_payment_clearing'
    ])
  );

-- 2. journal_lines metadata columns ---------------------------------------
ALTER TABLE public.journal_lines
  ADD COLUMN IF NOT EXISTS payment_method     text,
  ADD COLUMN IF NOT EXISTS source_amount      numeric,
  ADD COLUMN IF NOT EXISTS mapping_rule_type  text,
  ADD COLUMN IF NOT EXISTS mapping_match_key  text,
  ADD COLUMN IF NOT EXISTS mapping_status     text DEFAULT 'mapped';

-- 3. Rename existing KPAY accounts in place (preserves FK history) --------
UPDATE public.chart_of_accounts SET name = 'Payment Settlement Clearing – Assembly' WHERE code = '1290';
UPDATE public.chart_of_accounts SET name = 'Payment Settlement Clearing – Caliente' WHERE code = '1295';

-- Seed per-venue Payment Settlement Clearing account + mappings
DO $seed$
DECLARE
  v record;
  next_code int;
...
BEGIN
  SELECT COALESCE(MAX(NULLIF(regexp_replace(code,'\D','','g'),''))::int,0)+1
   ...
   WHERE code ~ '^12[0-9]+$';
  IF next_code < 1296 THEN next_code := 1296; END IF;

  FOR v IN SELECT name FROM public.venues WHERE is_active = true ORDER BY name LOOP
    SELECT id INTO v_acc_id
      FROM public.chart_of_accounts
     WHERE name = 'Payment Settlement Clearing – '||v.name
     LIMIT 1;

    IF v_acc_id IS NULL THEN
      INSERT INTO public.chart_of_accounts (code, name, account_type, normal_side, is_active, is_cash, description, sort_order)
        VALUES (next_code::text,
                'Payment Settlement Clearing – '||v.name,
                'asset','debit',true,false,
                'Electronic payments recorded as revenue, awaiting bank/processor settlement.',
                next_code)
    ...
    INSERT INTO public.account_mapping_rules (rule_type, match_key, account_id, notes)
      VALUES ('payment_settlement_clearing', v.name, v_acc_id,
              'All non-cash payment methods for this venue debit this account.')
      ON CONFLICT (rule_type, match_key) DO NOTHING;

    INSERT INTO public.account_mapping_rules (rule_type, match_key, account_id, notes)
      SELECT 'cash_on_hand', v.name, c.id, 'Cash receipts for this venue.'
...
-- 4. Rewrite rebuild_journal_from_operations ------------------------------
...
BEGIN
  ...
  LOOP
    ...
    -- DEBIT: each non-cash method -> Payment Settlement Clearing
    ...
    IF v_imbalance <> 0 AND acc_suspense IS NOT NULL THEN
      ...
      IF v_imbalance > 0 THEN
        INSERT INTO public.journal_lines (entry_id,account_id,debit,credit,venue,line_no,memo,mapping_rule_type,mapping_status)
          VALUES (e_id,acc_suspense,0,v_imbalance,r.venue,v_ln,'Δ','suspense','missing');
      ELSE
        INSERT INTO public.journal_lines (entry_id,account_id,debit,credit,venue,line_no,memo,mapping_rule_type,mapping_status)
          VALUES (e_id,acc_suspense,-v_imbalance,0,r.venue,v_ln,'Δ','suspense','missing');
      END IF;
      v_mapping_status := 'missing';
    END IF;
      ...
      UPDATE public.journal_entries
         SET status = v_entry_status,
             memo  = 'Sales summary — '||r.venue||
                     CASE WHEN v_mapping_status='missing' THEN ' (missing mapping)' ELSE '' END
       WHERE id = e_id;
      cnt := cnt+1;
    END IF;
  END LOOP;
...
-- 5. reverse_and_regenerate_sales_journal ---------------------------------
CREATE OR REPLACE FUNCTION public.reverse_and_regenerate_sales_journal(p_entry_id uuid)
 RETURNS jsonb
 LANGUAGE plpgsql
 SECURITY DEFINER
 SET search_path TO 'public'
...
BEGIN
  ...
  IF v_entry.source_type <> 'sales_summary' THEN
    RAISE EXCEPTION 'Only sales_summary entries can be reversed this way';
  END IF;

  SELECT display_name INTO v_uname FROM public.profiles WHERE user_id = v_uid LIMIT 1;

  INSERT INTO public.journal_entries (entry_date, memo, source_type, source_id, venue, status, manually_adjusted)
    VALUES (v_entry.entry_date,
            'Reversal of '||v_entry.memo, 'adjustment',
            'reversal_of:'||v_entry.id::text,
            v_entry.venue, 'posted', true)
    RETURNING id INTO v_new_id;

  INSERT INTO public.journal_lines (entry_id, account_id, debit, credit, venue, line_no, memo,
                                    payment_method, source_amount, mapping_rule_type, mapping_match_key, mapping_status)
  SELECT v_new_id, account_id, credit, debit, venue, line_no, 'Reversal: '||memo,
         payment_method, CASE WHEN source_amount IS NOT NULL THEN -source_amount END,
         mapping_rule_type, mapping_match_key, mapping_status
    FROM public.journal_lines WHERE entry_id = v_entry.id;

  UPDATE public.journal_entries SET status='void' WHERE id = v_entry.id;

  PERFORM public.rebuild_journal_from_operations();

  INSERT INTO public.ledger_audit_log (event_type, user_id, user_display_name, status, notes)
    VALUES ('sales_journal_reversed', v_uid, v_uname, 'success',
            'Reversed '||v_entry.id::text||' → '||v_new_id::text||' and re-generated draft');

  RETURN jsonb_build_object('success', true, 'reversal_entry_id', v_new_id);
END;
$$;

GRANT EXECUTE ON FUNCTION public.reverse_and_regenerate_sales_journal(uuid) TO authenticated;
