...
BEGIN
  ...
  LOOP
    ...
    INSERT INTO public.journal_lines (entry_id, account_id, debit, credit, venue, line_no)
      VALUES (e_id, acc_sales_cash, v_total, 0, r.venue, 1);
    INSERT INTO public.journal_lines (entry_id, account_id, debit, credit, venue, line_no)
      VALUES (e_id, acc_sales_default, 0, v_rev, r.venue, 2);
    IF v_svc > 0 AND acc_service IS NOT NULL THEN
      INSERT INTO public.journal_lines (entry_id, account_id, debit, credit, venue, line_no)
        VALUES (e_id, acc_service, 0, v_svc, r.venue, 3);
    END IF;
    UPDATE public.journal_entries SET status='posted' WHERE id=e_id;
    cnt := cnt + 1;
  END LOOP;
  ...
  LOOP
    ...
    END IF;
    -- reconcile rounding: adjust last line so debits == invoice total
