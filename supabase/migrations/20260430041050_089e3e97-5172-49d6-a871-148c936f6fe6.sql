...
-- 1. Tighten hr_shifts SELECT to admins/managers only
DROP POLICY IF EXISTS "Authenticated can read shifts" ON public.hr_shifts;
CREATE POLICY "Admins/managers can read shifts"
  ON public.hr_shifts FOR SELECT
  USING (has_role(auth.uid(), 'admin'::app_role) OR has_role(auth.uid(), 'manager'::app_role));

-- 2. Restrict realtime.messages: deny all by default (no realtime channels are used in this app)
DO $$ BEGIN
  IF EXISTS (SELECT 1 FROM pg_tables WHERE schemaname='realtime' AND tablename='messages') THEN
    EXECUTE 'ALTER TABLE realtime.messages ENABLE ROW LEVEL SECURITY';
    EXECUTE 'DROP POLICY IF EXISTS "Admins only realtime" ON realtime.messages';
    EXECUTE 'CREATE POLICY "Admins only realtime" ON realtime.messages FOR SELECT TO authenticated USING (public.has_role(auth.uid(), ''admin''::public.app_role))';
  END IF;
END $$;

-- 3. Lock down SECURITY DEFINER functions exposed via PostgREST
-- has_role: needed by RLS (which runs as table owner regardless), but should not be callable by anon
REVOKE EXECUTE ON FUNCTION public.has_role(uuid, app_role) FROM anon;

-- handle_new_user / handle_new_user_access are auth triggers — never call via API
REVOKE EXECUTE ON FUNCTION public.handle_new_user() FROM PUBLIC, anon, authenticated;
REVOKE EXECUTE ON FUNCTION public.handle_new_user_access() FROM PUBLIC, anon, authenticated;

-- rebuild_journal_from_operations: enforce admin-only at the function entry, and revoke anon
...
BEGIN
  -- Server-side authorisation: only admins may rebuild the ledger
