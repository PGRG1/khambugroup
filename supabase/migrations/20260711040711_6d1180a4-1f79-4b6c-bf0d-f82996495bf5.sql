-- =====================================================================
-- Finance section: subledger posting RPCs + rebuild patches + auto-fee
-- =====================================================================

-- 1) Track posted JEs on payments, credit_notes, and payment_allocations
ALTER TABLE public.payments
  ADD COLUMN IF NOT EXISTS journal_entry_id uuid REFERENCES public.journal_entries(id);
ALTER TABLE public.credit_notes
  ADD COLUMN IF NOT EXISTS journal_entry_id uuid REFERENCES public.journal_entries(id);
ALTER TABLE public.payment_allocations
  ADD COLUMN IF NOT EXISTS credit_note_journal_entry_id uuid REFERENCES public.journal_entries(id);

-- 2) Patched rebuild function: adds payroll_accrual/payroll_payment to
--    the drafts-delete list and adds per-processor fee lookup.
...
-- 3) Post AP payment (Dr AP / Cr bank) + optional credit-note offset JE.
...
BEGIN
  ...
  IF acc_bank IS NULL AND p.payment_method IS NOT NULL THEN
    SELECT account_id INTO acc_bank FROM public.account_mapping_rules
      WHERE tenant_id = v_tenant AND rule_type = 'payment_method_cash' AND match_key = p.payment_method LIMIT 1;
  END IF;
...
REVOKE ALL ON FUNCTION public.post_invoice_payment(uuid) FROM public;
GRANT EXECUTE ON FUNCTION public.post_invoice_payment(uuid) TO authenticated;

-- 4) Post payroll payment batch (Dr salary/MPF payable / Cr bank).
CREATE OR REPLACE FUNCTION public.post_payroll_batch(p_batch_id uuid)
RETURNS jsonb
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path TO 'public'
...
BEGIN
  ...
  v_tenant := b.tenant_id;
  IF v_tenant IS NULL THEN RAISE EXCEPTION 'Batch has no tenant_id'; END IF;
  ...
  IF b.journal_entry_id IS NOT NULL THEN
    RETURN jsonb_build_object('status','already_posted','journal_entry_id', b.journal_entry_id);
  END IF;

  v_amt := ROUND(COALESCE(b.total_amount,0)::numeric, 2);
  IF v_amt <= 0.005 THEN RAISE EXCEPTION 'Batch total is zero'; END IF;

  rule_key := CASE
    WHEN lower(COALESCE(b.payment_kind,'')) IN ('mpf','mpf_er','mpf_ee') THEN 'mpf_payable'
    ELSE 'salary_payable'
  END;
  SELECT account_id INTO acc_payable FROM public.account_mapping_rules
    WHERE tenant_id = v_tenant AND rule_type = rule_key LIMIT 1;
  IF acc_payable IS NULL THEN RAISE EXCEPTION 'Missing mapping rule: %', rule_key; END IF;

  IF b.bank_account_id IS NOT NULL THEN
    SELECT linked_gl_account_id INTO acc_bank FROM public.bank_accounts
      WHERE id = b.bank_account_id AND tenant_id = v_tenant;
  END IF;
  IF acc_bank IS NULL AND b.payment_method IS NOT NULL THEN
    SELECT account_id INTO acc_bank FROM public.account_mapping_rules
      WHERE tenant_id = v_tenant AND rule_type = 'payment_method_cash' AND match_key = b.payment_method LIMIT 1;
  END IF;
  IF acc_bank IS NULL THEN RAISE EXCEPTION 'No bank / cash account resolved'; END IF;

  SELECT display_name INTO v_uname FROM public.profiles WHERE user_id = v_uid LIMIT 1;

  INSERT INTO public.journal_entries (entry_date, memo, source_type, source_id, status, tenant_id, created_by, posted_at)
  VALUES (b.payment_date, 'Payroll batch · '||COALESCE(b.payment_kind,'payment')||' '||b.period_year||'-'||lpad(b.period_month::text,2,'0'),
          'payroll_payment', p_batch_id::text, 'posted', v_tenant, v_uid, now())
  RETURNING id INTO je_id;

  INSERT INTO public.journal_lines (entry_id, account_id, debit, credit, line_no, memo, tenant_id)
  VALUES (je_id, acc_payable, v_amt, 0, 1, 'Payroll liability cleared', v_tenant),
         (je_id, acc_bank,    0, v_amt, 2, 'Bank outflow',              v_tenant);

  UPDATE public.hr_payroll_payment_batches SET journal_entry_id = je_id WHERE id = p_batch_id;

  INSERT INTO public.ledger_audit_log (event_type, user_id, user_display_name, journal_entry_id, status, amount, notes, tenant_id)
  VALUES ('payroll_batch_posted', v_uid, v_uname, je_id, 'success', v_amt,
          'Posted payroll batch '||p_batch_id::text||' ('||b.payment_kind||')', v_tenant);

  RETURN jsonb_build_object('status','posted','journal_entry_id', je_id, 'amount', v_amt);
END;
$function$;

REVOKE ALL ON FUNCTION public.post_payroll_batch(uuid) FROM public;
GRANT EXECUTE ON FUNCTION public.post_payroll_batch(uuid) TO authenticated;

-- 5) Auto-post bank_fee JE on statement import for recognized fee lines.
CREATE OR REPLACE FUNCTION public.auto_post_bank_fee()
RETURNS trigger
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path TO 'public'
...
DECLARE
  ...
  je_id uuid;
  v_amt numeric;
  v_desc text := lower(COALESCE(NEW.description, '')||' '||COALESCE(NEW.reference,''));
BEGIN
  IF NEW.tenant_id IS NULL OR NEW.journal_entry_id IS NOT NULL THEN RETURN NEW; END IF;
  IF COALESCE(NEW.money_out,0) <= 0.005 THEN RETURN NEW; END IF;

  IF NOT (
    v_desc ~ 'bank\s*(charge|fee|charges|fees)'
    OR v_desc ~ 'service charge'
    OR v_desc ~ 'handling\s*(fee|charge)'
    OR v_desc ~ 'monthly\s*(fee|charge)'
    OR v_desc ~ 'account\s*maint'
    OR v_desc ~ 'remittance\s*(fee|charge)'
    OR v_desc ~ 'transfer\s*(fee|charge)'
  ) THEN
    RETURN NEW;
  END IF;

  SELECT account_id INTO acc_fee FROM public.account_mapping_rules
    WHERE tenant_id = NEW.tenant_id AND rule_type = 'bank_transfer_fee_default' LIMIT 1;
  IF acc_fee IS NULL THEN RETURN NEW; END IF;

  SELECT linked_gl_account_id INTO acc_bank FROM public.bank_accounts
    WHERE id = NEW.bank_account_id AND tenant_id = NEW.tenant_id;
  IF acc_bank IS NULL THEN RETURN NEW; END IF;

  v_amt := ROUND(NEW.money_out::numeric, 2);

  INSERT INTO public.journal_entries (entry_date, memo, source_type, source_id, status, tenant_id, posted_at)
  VALUES (NEW.txn_date, 'Bank fee — '||COALESCE(NEW.description,''), 'bank_fee', NEW.id::text, 'posted', NEW.tenant_id, now())
  RETURNING id INTO je_id;

  INSERT INTO public.journal_lines (entry_id, account_id, debit, credit, line_no, memo, tenant_id)
  VALUES (je_id, acc_fee,  v_amt, 0, 1, 'Bank fee expense', NEW.tenant_id),
         (je_id, acc_bank, 0, v_amt, 2, 'Bank outflow',     NEW.tenant_id);

  UPDATE public.bank_transactions
     SET journal_entry_id = je_id, status = 'bank_fee'
   WHERE id = NEW.id;

  INSERT INTO public.ledger_audit_log (event_type, journal_entry_id, status, amount, notes, tenant_id)
  VALUES ('bank_fee_posted', je_id, 'success', v_amt, 'Auto-posted bank fee for txn '||NEW.id::text, NEW.tenant_id);

  RETURN NEW;
END;
$function$;

DROP TRIGGER IF EXISTS trg_auto_post_bank_fee ON public.bank_transactions;
CREATE TRIGGER trg_auto_post_bank_fee
AFTER INSERT ON public.bank_transactions
FOR EACH ROW EXECUTE FUNCTION public.auto_post_bank_fee();

COMMENT ON TABLE public.account_mapping_rules IS
  'GL account routing. rule_type supports: sales_revenue, service_charge, sales_cash, sales_payment_method, sales_discount, tips_payable, payment_method_cash, invoice_expense, accounts_payable, payroll_salary_expense, payroll_mpf_expense, salary_payable, mpf_payable, manual_income, manual_expense, opening_equity, suspense, cash_payment_clearing, processor_fee_default, processor_fee (per-processor via match_key), bank_transfer_fee_default, invoice_discount, invoice_refund, payment_settlement_clearing, credit_note_clearing';
