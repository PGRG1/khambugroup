...
-- 1) Drop old unscoped RPC
DROP FUNCTION IF EXISTS public.rebuild_journal_from_operations();

-- 2) Create tenant-scoped RPC
...
BEGIN
  ...
  END IF;
  -- Membership check: caller must belong to the target tenant, or be platform admin
  ...
  DELETE FROM public.journal_lines
    WHERE tenant_id = p_tenant_id
      ...
      SELECT id FROM public.journal_entries
       WHERE tenant_id = p_tenant_id
         AND source_type IN ('sales_summary','invoice','invoice_payment','settlement_clearing','bank_txn')
         AND status = 'draft'
         AND COALESCE(manually_adjusted,false)=false
    );
...
REVOKE EXECUTE ON FUNCTION public.rebuild_journal_from_operations(uuid) FROM PUBLIC, anon;
GRANT EXECUTE ON FUNCTION public.rebuild_journal_from_operations(uuid) TO authenticated;

-- 3) Extend reporting views with tenant_id so PostgREST filters work
...
SELECT c.id AS account_id,
       ...
       CASE
         WHEN c.normal_side = 'debit'::text THEN COALESCE(sum(jl.debit), 0::numeric) - COALESCE(sum(jl.credit), 0::numeric)
         ELSE COALESCE(sum(jl.credit), 0::numeric) - COALESCE(sum(jl.debit), 0::numeric)
       END AS balance,
       c.tenant_id
  FROM public.chart_of_accounts c
  LEFT JOIN public.journal_lines  jl ON jl.account_id = c.id AND jl.tenant_id = c.tenant_id
  LEFT JOIN public.journal_entries je ON je.id = jl.entry_id AND je.status = 'posted'::text AND je.tenant_id = c.tenant_id
 GROUP BY c.id, c.code, c.name, c.account_type, c.normal_side, c.tenant_id
 ORDER BY c.code;
...
SELECT c.id AS account_id,
       ...
       CASE
         WHEN c.account_type = ANY (ARRAY['revenue'::text, 'other_income'::text]) THEN jl.credit - jl.debit
         ELSE jl.debit - jl.credit
       END AS amount,
       je.tenant_id
  FROM public.journal_lines  jl
  JOIN public.journal_entries je ON je.id = jl.entry_id AND je.status = 'posted'::text
  JOIN public.chart_of_accounts c ON c.id = jl.account_id
 WHERE c.account_type = ANY (ARRAY['revenue'::text, 'cogs'::text, 'opex'::text, 'other_income'::text, 'other_expense'::text]);
...
SELECT c.id AS account_id,
       ...
       je.entry_date,
       CASE
         WHEN c.normal_side = 'debit'::text THEN jl.debit - jl.credit
  ...
  JOIN public.journal_entries je ON je.id = jl.entry_id AND je.status = 'posted'::text
  JOIN public.chart_of_accounts c ON c.id = jl.account_id
 WHERE c.account_type = ANY (ARRAY['asset'::text, 'liability'::text, 'equity'::text]);
