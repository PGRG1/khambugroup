...
VALUES ('1900', 'Sales Reconciliation Suspense', 'asset', 'debit', false, 29,
        'Catches differences between recorded payment methods and total sales — investigate and clear.')
ON CONFLICT (code) DO NOTHING;

-- Update rebuild function to push any debit/credit imbalance into the suspense account
...
BEGIN
  ...
  LOOP
    ...
    -- Tip rounding drift -> push onto last card line
    ...
    -- Reconcile any remaining imbalance into Suspense
    ...
    IF v_imbalance <> 0 AND acc_suspense IS NOT NULL THEN
      ...
      IF v_imbalance > 0 THEN
        -- More debits than credits -> credit suspense
        INSERT INTO public.journal_lines (entry_id, account_id, debit, credit, venue, line_no, memo)
      ...
      END IF;
      UPDATE public.journal_entries SET memo = memo || ' (reconciliation Δ ' || to_char(v_imbalance, 'FM999,999,990.00') || ')' WHERE id = e_id;
    END IF;
