...
-- 1) Track the clearing journal on the batch & add a 'cleared' status
ALTER TABLE public.payment_settlement_batches
  ADD COLUMN IF NOT EXISTS clearing_journal_entry_id uuid
    REFERENCES public.journal_entries(id) ON DELETE SET NULL;

CREATE INDEX IF NOT EXISTS idx_psb_clearing_je
  ON public.payment_settlement_batches(clearing_journal_entry_id);

-- 2) Update the rebuild function: replace 'settlement_fee' block with full
--    'settlement_clearing' posting that fires only for matched batches.
...
BEGIN
  ...
  -- Clear the clearing-journal pointer for batches whose entry is about to be wiped
  ...
  -- ===== SETTLEMENT CLEARING (KPay / processor → bank) =====
  -- Only for batches MATCHED to a bank transaction.
  -- Dr Bank (net_settlement)
  -- Dr Processing fee expense (fee_amount)
  -- Dr Bank transfer fee expense (bank_transfer_fee)
  --     Cr Merchant Clearing (per payment_type, gross from settlement lines)
  -- Plug any residual to suspense (handles points_offset / adjustments / frozen).
  FOR sb IN
  ...
  LOOP
    -- Skip if user manually adjusted the existing clearing entry
    IF EXISTS (
      SELECT 1 FROM public.journal_entries
      WHERE source_type='settlement_clearing' AND source_id = sb.id::text AND manually_adjusted = true
    ) THEN CONTINUE; END IF;

    v_bank_amt := ROUND(COALESCE(NULLIF(sb.bank_money_in,0), sb.net_settlement, 0)::numeric, 2);
    ...
    IF v_gross <= 0 AND v_bank_amt <= 0 THEN CONTINUE; END IF;
    ...
    -- Fee accounts (merchant override → defaults)
    ...
    -- Dr Bank transfer / settlement fee
    ...
    -- Cr Merchant Clearing per payment_type using settlement lines
    FOR sl IN
    ...
    LOOP
      -- Strip _foreign suffix when looking up the mapping
      ...
      END IF;
      IF acc_pm IS NULL THEN
        -- No mapping: park to suspense so batch still clears
        acc_pm := acc_suspense;
      END IF;
      IF acc_pm IS NOT NULL THEN
        v_ln := v_ln+1;
    ...
    -- If no lines existed, fall back to single credit using gross_amount → suspense
    ...
    -- Plug residual to suspense (points_offset / adjustments / frozen / rounding)
    ...
    -- Mark the batch as cleared & link the journal entry
    UPDATE public.payment_settlement_batches
       SET clearing_journal_entry_id = e_id,
           status = 'cleared'
     WHERE id = sb.id;

    -- Also link the bank transaction to this entry (so it shows as journaled)
