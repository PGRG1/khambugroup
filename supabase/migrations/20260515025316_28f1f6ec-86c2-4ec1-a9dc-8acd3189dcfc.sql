...
SELECT 'payroll_salary_expense', '', id, 'Default seeded'
FROM public.chart_of_accounts WHERE code='6010'
  AND NOT EXISTS (SELECT 1 FROM public.account_mapping_rules WHERE rule_type='payroll_salary_expense' AND match_key='');

INSERT INTO public.account_mapping_rules (rule_type, match_key, account_id, notes)
SELECT 'payroll_mpf_expense', '', id, 'Default seeded'
FROM public.chart_of_accounts WHERE code='6020'
  AND NOT EXISTS (SELECT 1 FROM public.account_mapping_rules WHERE rule_type='payroll_mpf_expense' AND match_key='');
