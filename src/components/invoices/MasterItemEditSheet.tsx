          ...
          <div className="bani-visible-scrollbar flex-1 overflow-y-auto px-5 py-4 space-y-5">
            {loading ? <div className="flex min-h-48 items-center justify-center"><Loader2 className="h-5 w-5 animate-spin text-primary" /></div> : product ? <>
              <section className="space-y-3">
                ...
                <div className="grid grid-cols-1 gap-3 sm:grid-cols-2"><div><Label className="text-xs">Stock UOM (internal) *</Label><UomSelect type="stock" value={form.stock_uom} onChange={(v) => set("stock_uom", v)} legacyValues={[product.stock_uom || product.unit || ""]} allowCreate /><p className="mt-1 text-[10px] text-muted-foreground">Canonical internal unit; changing it updates the mirror on every supplier entry.</p></div><div><Label className="text-xs">Base / recipe UOM</Label><UomSelect type="base" value={form.base_unit_type} onChange={(v) => set("base_unit_type", v)} legacyValues={[product.base_unit_type || ""]} allowCreate /></div>{field("Base / recipe quantity *", form.base_unit_qty, (v) => set("base_unit_qty", v), { type: "number", min: 0.0001, step: "0.01" })}</div>
