...
export default function SourceDocumentViewer({ files, activeEvidenceField, evidence }: SourceDocumentViewerProps) {
  ...
  }, [applyManualZoom, activeUrl, open]);

  const goToPage = (next: number) => {
    if (nav.mode === "pdf") setPdfPage(clampPdfPage(next, pdfPageCount));
    else setPageIndex(Math.max(0, Math.min(files.length - 1, next - 1)));
  };

  const handleDocumentLoad = useCallback((count: number) => {
    setPdfPageCount(Math.max(1, count));
    setPdfPage((current) => clampPdfPage(current, Math.max(1, count)));
  }, []);

  const handlePageSize = useCallback((size: Size) => setNatural(size), []);
            ...
            <>
              <div className="flex flex-wrap items-center justify-between gap-2 border-b border-border/70 px-2 py-1.5">
                <div className="flex items-center gap-0.5">
                  <Button variant="ghost" size="icon" className="h-7 w-7" title="Previous page" aria-label="Previous page" disabled={nav.current <= 1} onClick={() => goToPage(nav.current - 1)}>
                    <ChevronLeft className="h-3.5 w-3.5" />
                  </Button>
                  <span className="min-w-[76px] text-center font-mono text-[11px] text-muted-foreground">{nav.current} / {nav.total}</span>
                  <Button variant="ghost" size="icon" className="h-7 w-7" title="Next page" aria-label="Next page" disabled={nav.current >= nav.total} onClick={() => goToPage(nav.current + 1)}>
                    ...
                    <div className="relative" style={natural.width ? { width: natural.width, height: natural.height } : undefined}>
                      {isPdf ? (
                        <PdfPageCanvas
                          ...
                          onOpenOriginal={() => window.open(activeUrl, "_blank", "noopener,noreferrer")}
                        />
                      ) : (
                        <img
                          src={activeUrl}
              ...
              {files.length > 1 && (
                <div className="bani-visible-scrollbar flex gap-1.5 overflow-x-auto border-t border-border/70 p-2 pb-2.5">
                  {files.map((file, index) => (
                    <Button key={`${file.name}-${index}`} variant={index === pageIndex ? "secondary" : "ghost"} size="sm" className="h-8 max-w-[150px] shrink-0 gap-1.5 px-2 text-[10px]" title={file.name} onClick={() => setPageIndex(index)}>
