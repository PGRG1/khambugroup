...
import type { usePettyCash } from "@/hooks/usePettyCash";
...
type ExtractedReceipt = {
  ...
  amount: number;
  receipt_date: string;
  suggested_classification_id: string;
  confidence: "high" | "medium" | "low";
  source_hint: string;
};

type ReviewRow = ExtractedReceipt & {
...
export default function PettyCashImport({
  open, onOpenChange, pc,
}: {
  open: boolean;
  onOpenChange: (o: boolean) => void;
  pc: ReturnType<typeof usePettyCash>;
}) {
  ...
  // Uploaded storage attachments per source file, indexed by files[] position.
  const [uploaded, setUploaded] = useState<Array<{ url: string | null; path: string | null }>>([]);

  const activeFloats = pc.floats.filter((f) => f.is_active);
  const activeClassifications = pc.classifications.filter((c) => c.is_active);
  const defaultFloatId = activeFloats[0]?.id ?? "";
  ...
  const runExtraction = async () => {
    if (files.length === 0 || !pc.tenantId) return;
    if (!defaultFloatId) {
      toast.error("No active petty cash float. Create one first.");
      return;
    }
    setScanning(true);
    try {
      // Upload each source file to storage in parallel (non-fatal per file).
      const { data: userData } = await supabase.auth.getUser();
      const uid = userData.user?.id;
      const uploads = await Promise.all(files.map(async (f) => {
        if (!uid) return { url: null, path: null };
        try {
      ...
      const classifications = activeClassifications.map((c) => ({ id: c.id, name: c.name }));

      const { data, error } = await supabase.functions.invoke("parse-petty-cash-receipt", {
        body: { files: payload, classifications },
      });
      if (error) throw error;
      if (!data?.success) throw new Error(data?.error || "Extraction failed");

      const extracted: ExtractedReceipt[] = Array.isArray(data.rows) ? data.rows : [];
      if (extracted.length === 0) {
        toast.warning("AI didn't find any receipts in the document(s).");
        setScanning(false);
        return;
      }
      const today = new Date().toISOString().slice(0, 10);
      setRows(extracted.map((r) => {
        // Map to a source file by parsing the source_hint if possible; else fall back to file 0.
        let src = 0;
        const m = r.source_hint?.match(/(?:image|page|row|file)\s*(\d+)/i);
        if (m) {
          const n = Number(m[1]);
          if (n > 0 && n <= files.length) src = n - 1;
        }
        return {
          ...r,
          classification_id: r.suggested_classification_id || "",
          float_id: defaultFloatId,
      ...
      toast.success(`AI extracted ${extracted.length} receipt${extracted.length === 1 ? "" : "s"}. Review before saving.`);
      setStep("review");
    } catch (e: any) {
  ...
  const validate = (r: ReviewRow): string | null => {
    ...
    if (!r.classification_id) return "Classification required";
    if (!r.float_id) return "Float required";
    if (!(r.amount > 0)) return "Amount must be > 0";
    if (!r.receipt_date) return "Date required";
    return null;
  };

  const saveRow = async (idx: number) => {
    const row = rows[idx];
    if (!row || !pc.tenantId) return;
    ...
    try {
      ...
      const attachment = uploaded[row.source_file_idx] ?? { url: null, path: null };
  ...
  const saveAll = async () => {
    ...
    if (!stillPending) toast.success("All receipts saved as pending for review.");
  };
      ...
      <DialogContent className="max-w-5xl max-h-[90vh] overflow-hidden flex flex-col">
        <DialogHeader>
          <DialogTitle className="flex items-center gap-2">
            ...
            AI Import — Petty Cash Receipts
          </DialogTitle>
          <DialogDescription>
            {step === "upload"
              ? "Upload petty cash receipts (photos, PDFs, or an Excel list). AI extracts each receipt for you to review before saving."
              : "Review each proposed receipt. Saved receipts land as pending — approve and post them manually from the list below."}
          </DialogDescription>
        </DialogHeader>
          ...
          <div className="space-y-4 overflow-y-auto">
            ...
            >
              ...
              <p className="text-xs text-muted-foreground mt-2">
                Images, PDF, or Excel (.xlsx/.xls). Max 15 MB each.
              </p>
            </div>
          ...
          <div className="overflow-y-auto space-y-2">
            {rows.length === 0 ? (
              ...
              <div className="space-y-2">
                {rows.map((r, idx) => {
                  ...
                  const unmatched = !r.classification_id;
                  return (
                    <div
                      key={idx}
                      className={
                        ...
                        (r.status === "saved" ? "border-primary/40 bg-primary/5"
                          ...
                          : unmatched ? "border-amber-500/40 bg-amber-500/5"
                    ...
                    >
                      <div className="flex items-center justify-between mb-2">
                        ...
                        <div className="flex items-center gap-1.5">
                          <Button size="sm" variant="outline" disabled={disabled} onClick={() => saveRow(idx)}>
                            {r.status === "saving" ? (
                              <><Loader2 className="h-3 w-3 mr-1 animate-spin" /> Saving</>
                            ) : "Save"}
                          </Button>
                          <Button size="sm" variant="ghost" disabled={disabled} onClick={() => removeRow(idx)}>
                      ...
                      <div className="grid grid-cols-1 sm:grid-cols-6 gap-2">
                        <div className="sm:col-span-2">
                          ...
                          <Input
                            ...
                            onChange={(e) => updateRow(idx, { receipt_date: e.target.value, status: "pending", error: undefined })}
                          />
                        </div>
                        <div className="sm:col-span-2">
                        ...
                        <div className="sm:col-span-2">
                          <Label className="text-[10px] uppercase tracking-wider text-muted-foreground">Float</Label>
                          <Select
                            value={r.float_id}
                            onValueChange={(v) => updateRow(idx, { float_id: v, status: "pending", error: undefined })}
                            disabled={disabled}
                          >
                        ...
                        <div className="sm:col-span-2">
                          ...
                          <Select
                            ...
                            onValueChange={(v) => updateRow(idx, { classification_id: v, status: "pending", error: undefined })}
                            disabled={disabled}
                          >
            ...
            {rows.length > 0 && (
              <div className="text-xs text-muted-foreground pt-2 border-t border-border/40">
                Total: {fmtHK(rows.reduce((s, r) => s + Number(r.amount || 0), 0))} across {rows.length} receipt(s)
                {uploaded.some((u) => u.url) && " · original file attached to each saved row"}
              </div>
            )}
          </div>
        )}
