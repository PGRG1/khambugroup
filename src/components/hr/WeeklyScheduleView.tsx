            ...
            <ResponsiveContainer width="100%" height="100%">
              <LineChart data={chartData} margin={{ top: 5, right: 10, left: 0, bottom: 5 }}>
                ...
                <RLegend
                  ...
                  onClick={(e: any) => {
                    const idx = weekDates.findIndex((d, i) => {
                      const label = `${d.toLocaleDateString("en-US", { month: "short", day: "numeric" })} ${DAY_NAMES[i]}`;
                  ...
                  formatter={(value: string, entry: any) => {
                    ...
                    const isHidden = chartDays.length > 0 && idx !== -1 && !chartDays.includes(idx);
                    return <span style={{ color: isHidden ? "#ccc" : entry.color, textDecoration: isHidden ? "line-through" : "none" }}>{value}</span>;
                  }}
                />
                {weekDates.map((d, i) => {
                  const hidden = chartDays.length > 0 && !chartDays.includes(i);
                  const dayLabel = `${d.toLocaleDateString("en-US", { month: "short", day: "numeric" })} ${DAY_NAMES[i]}`;
                  const colors = ["#6366f1", "#f59e0b", "#10b981", "#ef4444", "#8b5cf6", "#06b6d4", "#f97316"];
                  return <Line key={dayLabel} dataKey={dayLabel} stroke={colors[i % colors.length]} type="monotone" strokeWidth={hidden ? 0 : 2} dot={false} hide={hidden} />;
                })}
              </LineChart>
            </ResponsiveContainer>
          </div>
            ...
            <tbody>
              {hourlyCoverage.map(row => (
                <tr key={row.hour} className="border-b border-border/50 hover:bg-muted/30">
                  ...
                  {row.counts.map((c, i) => {
                    ...
                    const hBorder = getHolidayBorderClass(i, weekDates, holidayDates, formatDate);
                    return (
                      <td key={i} className={`${tdClass} text-center ${isHoliday ? "bg-destructive/5" : ""} ${isToday ? "bg-primary/5" : ""} ${hBorder} ${c === 0 ? "text-muted-foreground/40" : "font-medium"}`}>{c}</td>
                    );
                  })}
              ...
              <tr className="border-t-2 border-border bg-muted/50 font-bold">
                ...
                {weekDates.map((d, i) => {
                  ...
                  const hBorder = getHolidayBorderClass(i, weekDates, holidayDates, formatDate);
                  return (
                    <td key={i} className={`${tdClass} text-center font-bold ${isHoliday ? "bg-destructive/5" : ""} ${isToday ? "bg-primary/5" : ""} ${hBorder}`}>{dayTotal}</td>
                  );
                })}
