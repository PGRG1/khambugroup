  ...
  return (
    <Card className="p-4">
      <div className="flex items-center justify-between mb-3">
        ...
        <div className={`text-lg font-bold tabular-nums ${pctTone(totalPct)}`}>
        ...
        <div className="space-y-2">
          {rows.map((r) => (
            <div key={r.venue} className="flex items-center gap-3 text-xs">
              <div className="w-24 truncate font-medium">{r.venue}</div>
              ...
              <div className={`w-14 text-right tabular-nums font-semibold ${pctTone(r.labor_cost_pct)}`}>
                {r.labor_cost_pct != null ? `${r.labor_cost_pct.toFixed(1)}%` : "—"}
              </div>
