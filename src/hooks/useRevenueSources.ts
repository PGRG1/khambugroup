...
export function useRevenueSources() {
  ...
  const update = async (id: string, patch: Partial<Pick<RevenueSource, "name" | "description" | "is_active" | "sort_order">>) => {
    ...
    const { error } = await supabase.from("revenue_sources").update(cleaned).eq("id", id).eq("tenant_id", tenantId);
    if (error) {
  ...
  const remove = async (id: string) => {
    if (!tenantId) return false;
    const { error } = await supabase.from("revenue_sources").delete().eq("id", id).eq("tenant_id", tenantId);
    if (error) {
