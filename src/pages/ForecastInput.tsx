...
import { ArrowLeft, Plus, Trash2, MessageSquare, TrendingUp, TrendingDown, Minus } from "lucide-react";
...
const SALES_KEY = "khambu_sales_data";

function loadSalesData(): SalesRecord[] {
  try {
...
const ForecastInput = () => {
  ...
  const salesData = useMemo(() => loadSalesData(), []);
  ...
  const VarianceIndicator = ({ value, suffix = "" }: { value: number | null; suffix?: string }) => {
  ...
  return (
    <div className="min-h-screen bg-background p-4 sm:p-6 lg:p-8">
      <div className="max-w-[1400px] mx-auto space-y-6">
        ...
        <header className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-xs text-muted-foreground mt-0.5">Daily forecast entry & actuals comparison</p>
            </div>
          </div>
        ...
        {/* Forecast Table with Actuals Comparison */}
        <div className="card-glass rounded-xl p-6 animate-fade-in">
          <h3 className="text-sm font-display font-semibold text-foreground mb-4">
            Forecast vs Actuals — {venueName}
          </h3>
          {forecastsWithActuals.length === 0 ? (
            <p className="text-muted-foreground text-sm text-center py-8">
              No forecasts yet for {venueName}. Add one above!
            </p>
          ) : (
              ...
              <table className="w-full text-sm">
                <thead>
                  <tr className="border-b border-border">
                    ...
                    <th className="text-center py-2 px-2 text-xs font-semibold text-muted-foreground" colSpan={3}>
                      Customers
                    </th>
                    <th className="text-center py-2 px-2 text-xs font-semibold text-muted-foreground" colSpan={3}>
                      Avg Spend / Customer
                  ...
                  <tr className="border-b border-border/50">
                    ...
                    <th className="py-1 px-2"></th>
                    <th className="text-center py-1 px-2 text-[10px] text-muted-foreground font-medium">Forecast</th>
                    <th className="text-center py-1 px-2 text-[10px] text-muted-foreground font-medium">Actual</th>
                    <th className="text-center py-1 px-2 text-[10px] text-muted-foreground font-medium">Var</th>
                    <th className="text-center py-1 px-2 text-[10px] text-muted-foreground font-medium">Forecast</th>
                    <th className="text-center py-1 px-2 text-[10px] text-muted-foreground font-medium">Actual</th>
                    <th className="text-center py-1 px-2 text-[10px] text-muted-foreground font-medium">Var</th>
                    <th className="text-center py-1 px-2 text-[10px] text-muted-foreground font-medium">Forecast</th>
                  ...
                  {forecastsWithActuals.map((f) => (
                    <tr key={f.id} className="border-b border-border/30 hover:bg-muted/30 transition-colors">
                      ...
                      <td className="py-2.5 px-2 text-center">
                        {f.actualCustomers !== null ? f.actualCustomers : "—"}
                      </td>
                      <td className="py-2.5 px-2 text-center">
                        <VarianceIndicator value={f.customerVariance} />
                      </td>
                      {/* Avg Spend */}
                      <td className="py-2.5 px-2 text-center">{formatCurrency(f.forecastedAvgSpend)}</td>
                      <td className="py-2.5 px-2 text-center">
                        {f.actualAvgSpend !== null ? formatCurrency(f.actualAvgSpend) : "—"}
                      </td>
                      <td className="py-2.5 px-2 text-center">
                        <VarianceIndicator value={f.avgSpendVariance} />
                      </td>
                      {/* Total Sales */}
                      ...
                      <td className="py-2.5 px-2 text-center font-semibold">
                        {f.actualTotalSales !== null ? formatCurrency(f.actualTotalSales) : "—"}
                      </td>
                      <td className="py-2.5 px-2 text-center">
                        <VarianceIndicator value={f.totalSalesVariance} />
                      </td>
                      {/* Comment */}
