...
export default function Invoices() {
  ...
  const handleCreateInvoice = async () => {
    const lines = newLines.filter((l) => l.description.trim()).map((l) => {
      ...
      const price = parseFloat(l.unit_price) || 0;
      const tax = parseFloat(l.tax_amount) || 0;
      const weight = l.weight ? parseFloat(l.weight) : null;
      const lineTotal = qty * price + tax;
      return { item_code: l.item_code || "", description: l.description, pack_size: l.pack_size || "", category_id: null, quantity: qty, unit: l.unit || null, weight, unit_price: price, discount: 0, tax_amount: tax, total: lineTotal, notes: null, product_master_id: null };
    });
    const subtotal = lines.reduce((s, l) => s + l.total - l.tax_amount, 0);
    const taxTotal = lines.reduce((s, l) => s + l.tax_amount, 0);
      ...
      {scannerOpen && (
        <InvoiceScanner
          ...
          onSave={async (inv, lines, files) => {
            ...
            await createInvoice(
              { ...inv, discount: inv.discount ?? 0, status: inv.status || "pending", subtotal: lines.reduce((s, l) => s + l.total - l.tax_amount, 0), tax_amount: lines.reduce((s, l) => s + l.tax_amount, 0), total_amount: lines.reduce((s, l) => s + l.total, 0), entered_by: user?.id || "" },
              lines,
              fileUrl,
              fileName
            );
